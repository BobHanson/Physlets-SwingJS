...
public final class VectorField {
  ...
  public void paint(Graphics g, Rectangle r, int skip){
     int ppux=r.width/(col-1);
     int ppuy=r.height/(row-1);
     int ppu=(int)(Math.min(ppux,ppuy)*skip*(0.6));
     int i,j;
     int x1,y1,x2,y2,x,y,ytemp;
     double fx,fy,mag;
     double w=2;  // the size of the arrow head
     double h=15; // the length of the arrow
     for(i=0; i<row; i++) {
          ytemp = r.height-(int) (r.height*(double)i/(double)(row-1)) +r.y;
          for(j=0; j<col; j++) {
                y1=ytemp;
                x1 =(int) (r.width*(double)j/(double)(col-1) )+r.x;
                fx=field[i][j][0];
                fy=field[i][j][1];
                mag=field[i][j][2];
                ...
                double u=(w*x/(double)h);
                double v=(w*y/(double)h);
