...
//REMIND: huh?! this was written in a 'try and see' way (on Linux JDK-1.1.8)
    public void doLayout()
    {
        super.doLayout();
        scrollPane.doLayout();
        for ( int i = 0; i < scrollPane.getComponentCount(); i++ )
            scrollPane.getComponent( i ).doLayout(); 
    }
...
//        gridbagC.anchor  = GridBagConstraints.WEST;
        ...
        Enumeration enumEl = data.getElements();

        while ( enumEl.hasMoreElements() )
        {
            DataElement cdata = (DataElement)enumEl.nextElement();
