...
$I$(6).cacheFileData$S$O(this.path.name, this.path.秘bytes);
});
...
P$.JSFileSystem.ni$();
return Clazz.new_($I$(7).c$$BA$I$I$I$I$I$java_io_FileDescriptor,[this.bc.秘bytes, -1, (position|0), (size|0), (size|0), 0, this.fd]);
});
...
if (this.len < this.秘bytes.length) this.秘bytes=$I$(10).copyOf$BA$I(this.秘bytes, this.len);
this.path.秘bytes=this.秘bytes;
$I$(6).cacheFileData$S$O(this.path.name, this.秘bytes);
$I$(6).saveFile$S$O$S$S(this.path.name, this.秘bytes, null, null);
}});
...
return p$1._get$BA$I$I$I$Z.apply(bc, [this.秘bytes, fromPos, toPos, n, updatePos]);
});

Clazz.newMeth(C$, '_get$BA$I$I$I$Z', function (array, from, to, n, updatePos) {
if (to + n > p$1.getBytes.apply(this, []).length) this.秘bytes=$I$(11).ensureLengthByte$BA$I(this.秘bytes, (to + n) * 2);
...
if (this.秘bytes == null ) this.秘bytes=this.path.秘bytes;
if (this.秘bytes == null ) {
this.秘bytes=$I$(6).getFileAsBytes$O(this.path.toString());
