...
totalButtonWidth = totalButtonWidth+(pref.width);
}
if (this.getSyncAllWidths()) {
totalButtonWidth = maxWidth * numChildren;
}totalButtonWidth = totalButtonWidth+((numChildren - 1) * this.padding);
...
x = x+(children[index].getWidth() + this.padding);
}}
}});
...
aSize = children[counter].getPreferredSize();
height = Math.max(height, aSize.height);
totalWidth = totalWidth+(aSize.width);
}
totalWidth = totalWidth+(((numChildren - 1) * this.padding));
