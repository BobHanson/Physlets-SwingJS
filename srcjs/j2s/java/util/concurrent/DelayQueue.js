(function(){var P$=Clazz.newPackage("java.util.concurrent"),I$=[['java.util.concurrent.locks.ReentrantLock','java.util.PriorityQueue','java.util.concurrent.TimeUnit',['java.util.concurrent.DelayQueue','.Itr']]],$incl$=function(i){return I$[i]=Clazz.load(I$[0][i-1])};
...
this.lock = Clazz.new_((I$[1]||$incl$(1)));
this.available = this.lock.newCondition();
this.q = Clazz.new_((I$[2]||$incl$(2)));
}, 1);
...
this.q.offer$TE(e);
if (first == null  || e.compareTo$TT(first) < 0 ) this.available.signalAll();
return true;
} finally {
lock.unlock();
}
...
try {
var first = this.q.peek();
if (first == null  || first.getDelay$java_util_concurrent_TimeUnit((I$[3]||$incl$(3)).NANOSECONDS) > 0 ) return null;
 else {
var x = this.q.poll();
...
if (c == null ) throw Clazz.new_(Clazz.load('java.lang.NullPointerException'));
if (c === this ) throw Clazz.new_(Clazz.load('java.lang.IllegalArgumentException'));
var lock = this.lock;
lock.lock();
try {
var n = 0;
for (; ; ) {
var first = this.q.peek();
if (first == null  || first.getDelay$java_util_concurrent_TimeUnit((I$[3]||$incl$(3)).NANOSECONDS) > 0 ) break;
...
return Clazz.new_((I$[4]||$incl$(4)).c$$OA, [this, null, this.toArray()]);
});
