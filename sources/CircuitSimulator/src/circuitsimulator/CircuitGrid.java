...
/**
 * This class is the engine of the circuit applet. The circuit grid is
 * constructed here with all its components. It contains all circuit evaluation
...
public class CircuitGrid {
	...
	GridElement[][] element;
	Vector<VEquation> vEquations = new Vector<>(); // vector of V(I) characteristics for each CircuitElement
	private Vector<BranchPoint> branchPoints = new Vector<>(); // vector of GridElements with 3 or 4 connections
	// private Vector equalVs = new Vector(); // vector of V indices which are equal
	// to each other
	Vector<CircuitElement> cirElemList = new Vector<>(); // vector of CircuitElements
	Vector<Source> sourceContainer = new Vector<>(); // vector of Sources
	private Vector<Constraints> constraints = new Vector<>(); // vector of constraints between separate circuits
			...
			do {
				if (element[currow][curcol].defined == true) {
					if (element[currow][curcol].nOfUntreatedCons > 2)
						...
						firstLoopCorrections(); // procedure for the first loop
	...
	public void reset() {
		...
		for (int i = 0; i <= numberOfPars + sourceContainer.size(); i++)
			y[i] = 0.0;
	}
	...
	private boolean initDefinition() {
		...
		while (element[currow][curcol].numberOfCons() == 0) {
			currow = 0;
			while (element[currow][curcol].numberOfCons() == 0 && currow < rows - 1)
				currow++;
			if (element[currow][curcol].numberOfCons() == 0)
				curcol++;
		}
		if ((circuit.debugLevel & Circuit.DEBUG_NUM) > 0)
			System.out.println("first element found: " + currow + "," + curcol);
	...
	private boolean findNextCircuit() {
		...
		while ((element[currow][curcol].numberOfCons() == 0) || element[currow][curcol].defined) {
			...
			while (((element[currow][curcol].numberOfCons() == 0) || element[currow][curcol].defined)
					&& (currow < rows - 1))
				currow++;
			if ((element[currow][curcol].numberOfCons() == 0) || element[currow][curcol].defined)
				curcol++;
			if (curcol == cols - 1) {
				if ((circuit.debugLevel & Circuit.DEBUG_NUM) > 0)
		...
		if ((circuit.debugLevel & Circuit.DEBUG_NUM) > 0)
			System.out.println("next circuit found: " + currow + "," + curcol);
		direction = 3;// put direction up to find first connection clockwise in a later stage
		lastVIndex++;
		lastIIndex++;
		startVIndex = lastVIndex;
		startIIndex = lastIIndex;
		// numberOfCirc++;
	...
	private boolean startNextBranchPoint() { // find next BranchPoint and start new loop
		...
		if (branchPoints.size() > 0) {
			...
			if (nextBranchPoint < branchPoints.size()) {
				thisBranchPoint = branchPoints.elementAt(nextBranchPoint);
				currow = thisBranchPoint.xcoord;
				curcol = thisBranchPoint.ycoord;
				curVIndex = element[currow][curcol].getVIndex();
				direction = thisBranchPoint.start(element[currow][curcol], lastIIndex);
				if ((circuit.debugLevel & Circuit.DEBUG_NUM) > 0)
					System.out.println("next branch started at " + currow + "," + curcol + ", lastVIndex/lastIIndex: "
							+ lastVIndex + "/" + lastIIndex);
				if ((circuit.debugLevel & Circuit.DEBUG_NUM) > 0)
					thisBranchPoint.print();
				lastIIndex++;
	...
	private void stopBranchCorrections() {
		...
		if (((VEquation) vEquations.elementAt(bpStartVEquation)).indexV1 != prevVIndex) {
			...
			if ((circuit.debugLevel & Circuit.DEBUG_NUM) > 0)
				System.out.println(
						"Equality for V-indices: " + prevVIndex + "=" + curVIndex + ", with I-index=" + lastIIndex);
		}
		if ((circuit.debugLevel & Circuit.DEBUG_NUM) > 0)
			branchPointPtr.print();
	}
	...
	private void addNewVEquation() { // prepare a V-equation for the actual element
		if (element[currow][curcol].connection[direction].getMyName().equals("wire") == false) {
			lastVIndex++;
			vEquations.addElement(new VEquation(element[currow][curcol], direction, curVIndex, lastVIndex, lastIIndex));
			curVIndex = lastVIndex;
			// check linearity of element
			if (element[currow][curcol].connection[direction].leftlinear == false)
				leftlinear = false;
			if (element[currow][curcol].connection[direction].rightlinear == false)
				rightlinear = false;
		} else {
			vEquations.addElement(new VEquation(element[currow][curcol], direction, curVIndex, curVIndex, lastIIndex));
		}
		vEquationPtr = vEquations.lastElement();
	...
	private void addNewBranchPoint() {
		branchPoints
				.addElement(new BranchPoint(element[currow][curcol], currow, curcol, dirold, direction, lastIIndex));
		branchPointPtr = branchPoints.lastElement();
	...
	private void findNextDirection() {
		// look for the next untreated connection in a grid point, turning
		// counter-clockwise
		dirold = direction;
	...
	private void leftEquationMatrix(Matrix matrix) {
		...
		for (int i = 0; i < vEquations.size(); i++) { // build v equations
			vEquationPtr = vEquations.elementAt(i);
			if (!vEquationPtr.z.getMyName().equals("wire"))
		...
		for (int i = 0; i < constraints.size(); i++) {
			constraintPtr = constraints.elementAt(i);
		...
		if (!branchPoints.isEmpty())
			for (int i = startint; i < numberOfPars; i++) {
				branchPointPtr = branchPoints.elementAt(i - startint);
				for (int j = 0; j < 4; j++)
...
//        CircuitElement cirelem;
	...
	public void calculateStep(double t) { // used for next step calculations
		...
		for (int i = 1; i <= sourceContainer.size(); i++)
			y[numberOfPars + i] = sourceContainer.elementAt(i - 1).getV(t);
		if (leftlinear == false) {
			leftEquationMatrix(a);
			aInv.matinv(a);
		}
		if (rightlinear == false)
			rightEquationMatrix(b);
		if (leftlinear == false || rightlinear == false)
			multip.matmul(aInv, b);
	...
	public int addCircuitElement(CircuitElement cirelem) {
		...
		if ((tor < rows) & (fromr < rows) & (toc < cols) & (fromc < cols) & (tor >= 0) & (toc >= 0) & (fromr >= 0)
				& (fromr >= 0)) {
			if (element[tor][toc].connection[tocon] != null)
				cirElemList.removeElement(element[tor][toc].connection[tocon]);
			if (cirelem.getMyName().equals("nothing") == false) {
				element[tor][toc].connection[tocon] = element[fromr][fromc].connection[fromcon] = cirelem;
				cirElemList.addElement(cirelem);
			} else
				element[tor][toc].connection[tocon] = element[fromr][fromc].connection[fromcon] = null;
	...
	public boolean removeCircuitElement(int id) {
		...
		if ((circuit.debugLevel & Circuit.DEBUG_IO) > 0)
			System.out.println("Element removed: " + cirelem.getMyName());
		return true;
	}
	...
	public boolean moveCircuitElement(int id, int newfromr, int newfromc, String to) {
		...
		newtor = newfromr;
		newtoc = newfromc; // make new connections
		if (to.equals("h")) {
			newfromcon = 2;
			newtocon = 0;
			newtoc++;
		...
		if ((newtor < rows) & (newfromr < rows) & (newtoc < cols) & (newfromc < cols) & (newtor >= 0) & (newtoc >= 0)
				& (newfromr >= 0) & (newfromr >= 0)) {
			element[tor][toc].connection[tocon] = element[fromr][fromc].connection[fromcon] = null;
			...
			if ((circuit.debugLevel & Circuit.DEBUG_IO) > 0)
				System.out.println(cirelem.getMyName() + " moved to: " + newfromr + " " + newfromc + " " + to);
			element[newtor][newtoc].connection[newtocon] = element[newfromr][newfromc].connection[newfromcon] = cirelem;
			return true;
		} else
			return false;
	}
	...
	public CircuitElement getCircuitElement(int id) {
		for (Enumeration<CircuitElement> e = cirElemList.elements(); e.hasMoreElements();) {
			CircuitElement cirelem = e.nextElement();
			if (cirelem.hashCode() == id)
				return cirelem;
		}
		if (Circuit.DEBUG)
	...
	public CircuitElement getCircuitElement(int r, int c, String tostr) {
		for (Enumeration<CircuitElement> e = cirElemList.elements(); e.hasMoreElements();) {
			CircuitElement cirelem = e.nextElement();
			if (cirelem.row == r & cirelem.col == c & cirelem.to.equals(tostr))
				return cirelem;
		}
		if (Circuit.DEBUG)
			System.out.println("getCircuitElement method: No element found");
