(function(){var P$=Clazz.newPackage("sun.awt.geom"),I$=[['sun.awt.geom.Order0','sun.awt.geom.Order1','sun.awt.geom.Order2','sun.awt.geom.Order3','java.lang.InternalError']],$incl$=function(i){return I$[i]=Clazz.load(I$[0][i-1])};
...
curves.add$TE(Clazz.new_((I$[1]||$incl$(1)).c$$D$D,[x, y]));
}, 1);

Clazz.newMeth(C$, 'insertLine$java_util_Vector$D$D$D$D', function (curves, x0, y0, x1, y1) {
if (y0 < y1 ) {
curves.add$TE(Clazz.new_((I$[2]||$incl$(2)).c$$D$D$D$D$I,[x0, y0, x1, y1, 1]));
} else if (y0 > y1 ) {
curves.add$TE(Clazz.new_((I$[2]||$incl$(2)).c$$D$D$D$D$I,[x1, y1, x0, y0, -1]));
} else {
}}, 1);
...
var y1 = coords[3];
if (y0 > y1 ) {
(I$[3]||$incl$(3)).insert$java_util_Vector$DA$D$D$D$D$D$D$I(curves, coords, coords[2], y1, coords[0], coords[1], x0, y0, -1);
} else if (y0 == y1  && y0 == coords[1]  ) {
return;
} else {
(I$[3]||$incl$(3)).insert$java_util_Vector$DA$D$D$D$D$D$D$I(curves, coords, x0, y0, coords[0], coords[1], coords[2], y1, 1);
}}, 1);

Clazz.newMeth(C$, 'insertCubic$java_util_Vector$D$D$DA', function (curves, x0, y0, coords) {
var y1 = coords[5];
if (y0 > y1 ) {
(I$[4]||$incl$(4)).insert$java_util_Vector$DA$D$D$D$D$D$D$D$D$I(curves, coords, coords[4], y1, coords[2], coords[3], coords[0], coords[1], x0, y0, -1);
} else if (y0 == y1  && y0 == coords[1]   && y0 == coords[3]  ) {
return;
} else {
(I$[4]||$incl$(4)).insert$java_util_Vector$DA$D$D$D$D$D$D$D$D$I(curves, coords, x0, y0, coords[0], coords[1], coords[2], coords[3], coords[4], y1, 1);
}}, 1);
...
throw Clazz.new_((I$[5]||$incl$(5)).c$$S,["no s progress!"]);
}if (t1 - t0 > 0.001 ) {
var t = (t0 + t1) / 2;
...
throw Clazz.new_((I$[5]||$incl$(5)).c$$S,["no t progress!"]);
}if (ys >= yt0  && yt >= ys0  ) {
