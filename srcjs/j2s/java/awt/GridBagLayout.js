(function(){var P$=Clazz.newPackage("java.awt"),I$=[['java.util.Hashtable','java.awt.GridBagConstraints','java.awt.Point','java.awt.Dimension','java.awt.GridBagLayoutInfo','java.util.Arrays',['java.awt.Component','.BaselineResizeBehavior'],'java.awt.Rectangle']],$incl$=function(i){return I$[i]=Clazz.load(I$[0][i-1])};
...
C$.$init$.apply(this);
this.comptable = Clazz.new_((I$[1]||$incl$(1)));
this.defaultConstraints = Clazz.new_((I$[2]||$incl$(2)));
}, 1);
...
var origin = Clazz.new_((I$[3]||$incl$(3)).c$$I$I,[0, 0]);
...
var loc = Clazz.new_((I$[3]||$incl$(3)).c$$I$I,[0, 0]);
var i;
var d;
...
throw Clazz.new_(Clazz.load('java.lang.IllegalArgumentException').c$$S,["cannot add to layout: constraints must be a GridBagConstraint"]);
}});

Clazz.newMeth(C$, 'removeLayoutComponent$java_awt_Component', function (comp) {
p$.removeConstraints$java_awt_Component.apply(this, [comp]);
});
...
return Clazz.new_((I$[4]||$incl$(4)).c$$I$I,[2147483647, 2147483647]);
});
...
curRow = curCol = -1;
var arraySizes = p$.preInitMaximumArraySizes$java_awt_Container.apply(this, [parent]);
...
if (p$.calculateBaseline$java_awt_Component$java_awt_GridBagConstraints$java_awt_Dimension.apply(this, [comp, constraints, d])) {
hasBaseline = true;
...
r = Clazz.new_((I$[5]||$incl$(5)).c$$I$I,[layoutWidth, layoutHeight]);
curRow = curCol = -1;
(I$[6]||$incl$(6)).fill$IA$I(xMaxArray, 0);
(I$[6]||$incl$(6)).fill$IA$I(yMaxArray, 0);
var maxAscent = null;
var maxDescent = null;
...
if (constraints.baselineResizeBehavior === (I$[7]||$incl$(7)).CONSTANT_DESCENT ) {
maxDescent[curY + curHeight - 1] = Math.max(maxDescent[curY + curHeight - 1], constraints.descent);
} else {
maxAscent[curY] = Math.max(maxAscent[curY], constraints.ascent);
}}if (constraints.baselineResizeBehavior === (I$[7]||$incl$(7)).CONSTANT_DESCENT ) {
baselineType[curY + curHeight - 1] = (baselineType[curY + curHeight - 1]|((1 << constraints.baselineResizeBehavior.ordinal()))|0);
} else {
baselineType[curY] = (baselineType[curY]|((1 << constraints.baselineResizeBehavior.ordinal()))|0);
}}break;
case 1024:
case 1280:
case 1536:
...
for (k = constraints.tempX; k < px; k++) pixels_diff = pixels_diff-(r.minWidth[k]);
...
r.minWidth[k] = r.minWidth[k]+(dx);
pixels_diff = pixels_diff-(dx);
weight -= wt;
}
r.minWidth[px - 1] = r.minWidth[px - 1]+(pixels_diff);
...
} else if (constraints.baselineResizeBehavior !== (I$[7]||$incl$(7)).CONSTANT_DESCENT ) {
...
}for (k = constraints.tempY; k < py; k++) pixels_diff = pixels_diff-(r.minHeight[k]);
...
r.minHeight[k] = r.minHeight[k]+(dy);
pixels_diff = pixels_diff-(dy);
weight -= wt;
}
r.minHeight[py - 1] = r.minHeight[py - 1]+(pixels_diff);
...
constraints.ascent = constraints.ascent+(constraints.insets.top);
constraints.baselineResizeBehavior = c.getBaselineResizeBehavior();
constraints.centerPadding = 0;
if (constraints.baselineResizeBehavior === (I$[7]||$incl$(7)).CENTER_OFFSET ) {
...
r.x = r.x-(r.width - constraints.insets.right);
...
case 256:
r.x = r.x+((diffx/2|0));
p$.alignOnBaseline$java_awt_GridBagConstraints$java_awt_Rectangle$I$I.apply(this, [constraints, r, cellY, cellHeight]);
break;
case 512:
if (this.rightToLeft) {
r.x = r.x+(diffx);
...
case 1024:
r.x = r.x+((diffx/2|0));
p$.alignAboveBaseline$java_awt_GridBagConstraints$java_awt_Rectangle$I$I.apply(this, [constraints, r, cellY, cellHeight]);
break;
case 1280:
if (this.rightToLeft) {
r.x = r.x+(diffx);
...
case 1792:
r.x = r.x+((diffx/2|0));
p$.alignBelowBaseline$java_awt_GridBagConstraints$java_awt_Rectangle$I$I.apply(this, [constraints, r, cellY, cellHeight]);
break;
case 2048:
if (this.rightToLeft) {
r.x = r.x+(diffx);
...
break;
default:
throw Clazz.new_(Clazz.load('java.lang.IllegalArgumentException').c$$S,["illegal anchor value"]);
}
});

Clazz.newMeth(C$, 'alignOnBaseline$java_awt_GridBagConstraints$java_awt_Rectangle$I$I', function (cons, r, cellY, cellHeight) {
if (cons.ascent >= 0) {
if (cons.baselineResizeBehavior === (I$[7]||$incl$(7)).CONSTANT_DESCENT ) {
...
}if (cons.baselineResizeBehavior === (I$[7]||$incl$(7)).OTHER ) {
var fits = false;
ascent = this.componentAdjusting.getBaseline$I$I(r.width, r.height);
if (ascent >= 0) {
ascent = ascent+(cons.insets.top);
...
if (ascent2 >= 0) {
ascent2 = ascent2+(cons.insets.top);
...
case (I$[7]||$incl$(7)).OTHER:
break;
default:
break;
}
}}} else {
p$.centerVertically$java_awt_GridBagConstraints$java_awt_Rectangle$I.apply(this, [cons, r, cellHeight]);
}});
...
if (!cons.isVerticallyResizable()) {
r.y = r.y+(Math.max(0, ((cellHeight - cons.insets.top - cons.insets.bottom - cons.minHeight - cons.ipady )/2|0)));
}});
...
Clazz.newMeth(C$, 'GetMinSize$java_awt_Container$java_awt_GridBagLayoutInfo', function (parent, info) {
var d = Clazz.new_((I$[4]||$incl$(4)));
var i;
var t;
var insets = parent.getInsets();
t = 0;
for (i = 0; i < info.width; i++) t = t+(info.minWidth[i]);

d.width = t + insets.left + insets.right ;
t = 0;
for (i = 0; i < info.height; i++) t = t+(info.minHeight[i]);
...
p$.arrangeGridPriv$java_awt_Container.apply(this, [parent]);
});
...
info.minWidth[i] = info.minWidth[i]+(dx);
r.width = r.width+(dx);
if (info.minWidth[i] < 0) {
r.width = r.width-(info.minWidth[i]);
info.minWidth[i] = 0;
}}
}diffw = parent.width - r.width;
} else {
...
info.minHeight[i] = info.minHeight[i]+(dy);
r.height = r.height+(dy);
if (info.minHeight[i] < 0) {
r.height = r.height-(info.minHeight[i]);
info.minHeight[i] = 0;
}}
}diffh = parent.height - r.height;
} else {
...
if (!this.rightToLeft) {
r.x = info.startx;
for (i = 0; i < constraints.tempX; i++) r.x = r.x+(info.minWidth[i]);
...
for (i = 0; i < constraints.tempY; i++) r.y = r.y+(info.minHeight[i]);
