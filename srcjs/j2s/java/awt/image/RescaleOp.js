(function(){var P$=Clazz.newPackage("java.awt.image"),I$=[['java.awt.image.ByteLookupTable','swingjs.JSUtil','java.awt.image.LookupOp','java.awt.image.BufferedImage',['java.awt.geom.Point2D','.Float']]],$incl$=function(i){return I$[i]=Clazz.load(I$[0][i-1])};
...
throw Clazz.new_(Clazz.load('java.lang.IllegalArgumentException').c$$S,["Number of bands in src " + numBands + " does not equal number of bands in dest " + dst.getNumBands() ]);
}if (this.length != 1 && this.length != src.getNumBands() ) {
throw Clazz.new_(Clazz.load('java.lang.IllegalArgumentException').c$$S,["Number of scaling constants does not equal the number of of bands in the src raster"]);
}if (p$.canUseLookup$java_awt_image_Raster$java_awt_image_Raster.apply(this, [src, dst])) {
...
var lut = p$.createByteLut$FA$FA$I$I.apply(this, [this.scaleFactors, this.offsets, numBands, srcNgray]);
var op = Clazz.new_((I$[3]||$incl$(3)).c$$java_awt_image_LookupTable$java_awt_RenderingHints,[lut, this.hints]);
