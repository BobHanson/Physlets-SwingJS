 ...
 *                                                                                        *
 * Copyright (c) 1996-97 Yanto Suryono. All Rights Reserved.                              *
 *                                                                                        *
...
// end of W. Christian additions.
  ...
  private Object vertexLock=new Object();    // the dummy object to lock when switching the drawing vertex.
   ...
   * @see   #setContour
   */
  public void setNoDrawing(boolean noDrawing) {
    synchronized(runLock){
  ...
  public void setDataAvailability(boolean avail) {
    ...
    if(avail){ // added by W. Christian to force a new image if we have new data.
        image_drawn = false;
        repaint();
    }
  }
 ...
 /*
  public void setValuesArray(SurfaceVertex[][] vertex) {
    boolean temp=rotate;
    stopRotation();
    synchronized(runLock){   // make sure the drawing has finished and we are in a wait state.
        this.vertex = vertex;
    }
    rotate=temp;
  ...
  */
  ...
  public SurfaceVertex[][] setNewData(SurfaceVertex[][] newVertex) {
    synchronized(vertexLock){
        if(this.vertex_new==null) this.vertex=newVertex;  // make sure we have a valid vertex.
        this.vertex_new = newVertex;
        data_available = true;
        is_data_available = true;  // see Handlers for mouse input events section
        image_drawn = false;
        repaint();
        return this.vertex;
    }
  ...
  private void syncNewData(){
    synchronized(vertexLock){
       if(vertex_new!=null) vertex=vertex_new;
    }
  }
  ...
  public boolean mouseUp(Event e, int x, int y) {
    ...
    //if (controller.isExpectDelay() && dragged) {      // always repaint
      destroyImage();
      data_available = is_data_available;    // restore the original data availabe flag
      repaint();
      dragged = false;
    //}
    mouseDown=false;
  ...
  public void paint(Graphics g) {
    ...
    **/
    ...
    if (data_available && !interrupted) {
      ...
      if(Buffer==null){  // don't change the image if it exists!
          g.setColor(Color.lightGray);
          g.fillRect(0,0,getBounds().width,getBounds().height);
  ...
  private boolean doCalc(){    // return false if we should stop running
      ...
      if (contour) plotContour();
          ...
          if (plot_mode == WIREFRAME) plotWireframe();
              else plotSurface();
        }
 ...
 *----------------------------------------------------------------------------------------*/
  ...
  private void importVariables() {
    plot_mode = controller.getPlotMode();
    if(mouseDown && !contour && ! density )plot_mode=0;  // wireframe while dragging.
    isBoxed = controller.isBoxed();
    isMesh = controller.isMesh();
    isScaleBox = controller.isScaleBox();
    isDisplayXY = controller.isDisplayXY();
    isDisplayZ = controller.isDisplayZ();
    isDisplayGrids = controller.isDisplayGrids();
    calc_divisions = controller.getCalcDivisions();
    plotfunc1 = controller.isPlotFunction1() || (dataGenerator.isExternalData());
    plotfunc2 = controller.isPlotFunction2();
 ...
 *----------------------------------------------------------------------------------------*/
    ...
    for (int counter=0; counter <= 4; counter++) {
      if (values1[i].z < values2[i].z) {
        ...
        if (trackposition != UPPER) {
          ...
          lowerpart[lowercount++] = new SurfaceVertex(xi,yi,zi,this);
          upperpart[uppercount++] = values2[i];
      ...
      if (values1[i].z > values2[i].z) {
        ...
        if (trackposition != LOWER) {
          ...
          upperpart[uppercount++] = new SurfaceVertex(xi,yi,zi,this);
          lowerpart[lowercount++] = values2[i];
  ...
  final  Point[] testpoint = new Point[5];
  ...
  private final void plotSurface() {
    ...
    image_drawn = false;
    try {
      zi = controller.getZMin();
      //zx = Float.valueOf(frame.getZMax()).floatValue();
      zx=controller.getZMax();
    ...
    int plot_density = controller.getDispDivisions();
    int multiple_factor = calc_divisions / plot_density;
    controller.setDispDivisions(plot_density);

    //paintThread.yield();
    zmin = zi; zmax = zx;
    if (rotate){
         //frame.setMessage("rotating ...");
         setMessage("rotating ...");
    }else{
    ...
    if (!plotfunc1 && !plotfunc2) {
      ...
      if (!rotate) setMessage("completed");
      image_drawn = true;
      return;
    }

    //SurfaceVertex.setZRange(zmin,zmax);
    zmaxV=zmax; zminV=zmin;
  ...
  private final void computePlotArea() {
    setAxesScale();
    contour_lines = controller.getContourLines();
    ...
    //width *= 0.9f;
    //height *= 0.9f;    use gutter variables instead.

    width -= gutter;     //added by W. Christian
    height -= gutter;
  ...
  /**
   *   Create a spectrum for the contour colors.
  */
  void setContourColor(){  // Creates color bank
    contour_color = new Color[contour_lines+1];
    for (int i=0; i <= contour_lines; i++) {
      float level = (float)i / contour_lines * 0.8f;
      int r=0,b=0;
      switch (plot_mode) {
        case DUALSHADE:   r=(Math.max(0,-contour_lines+i*2)*255)/contour_lines;
                          b=(Math.max(0,contour_lines-i*2)*255)/contour_lines;
                          contour_color[i] = new Color(r,0,b);
                          break;
        ...
        default:
                          contour_color[i] = Color.getHSBColor(0,0,level);
                          break;
  ...
  private final void plotContour() {
    ...
    // Contour lines
    if (plot_mode==DUALSHADE)  BufferGC.setColor(new Color(0,64,0)); // dark green
        else BufferGC.setColor(Color.black);
  ...
  private final void plotDensity() {
    ...
    image_drawn = false;
    try {
      zi =controller.getZMin(); // Float.valueOf(frame.getZMin()).floatValue();
      zx =controller.getZMax(); //valueOf(frame.getZMax()).floatValue();
      if (zi >= zx) throw new NumberFormatException();
    }
    catch(NumberFormatException e) {
      //frame.setMessage("Error in ranges");
  ...
  private final void plotWireframe() {
    ...
    try {
      zi = controller.getZMin(); //Float.valueOf(frame.getZMin()).floatValue();
      zx = controller.getZMax();//Float.valueOf(frame.getZMax()).floatValue();
      if (zi >= zx) throw new NumberFormatException();
    }
    catch(NumberFormatException e) {
      //frame.setMessage("Error in ranges");
    ...
    plot_density = controller.getDispDivisions();
   ...
   // paintThread.yield();
    ...
    if (isBoxed) drawBoundingBox();
    if (!rotate) setMessage("completed");  //frame.setMessage("completed");
    image_drawn = true;
  }
}
