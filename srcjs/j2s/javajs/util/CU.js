(function(){var P$=Clazz.newPackage("javajs.util"),I$=[['java.util.Hashtable','javajs.util.PT','javajs.util.P3']],$incl$=function(i){return I$[i]=Clazz.load(I$[0][i-1])};
...
C$.mapJavaScriptColors = Clazz.new_((I$[1]||$incl$(1)));
{
for (var i = C$.colorNames.length; --i >= 0; ) C$.mapJavaScriptColors.put$TK$TV(C$.colorNames[i], Integer.$valueOf(C$.colorArgbs[i]));
...
var tokens = (I$[2]||$incl$(2)).split$S$S(strColor.substring(1, strColor.length$() - 1), ",");
if (tokens.length != 3) return 0;
var red = (I$[2]||$incl$(2)).parseFloat$S(tokens[0]);
var grn = (I$[2]||$incl$(2)).parseFloat$S(tokens[1]);
var blu = (I$[2]||$incl$(2)).parseFloat$S(tokens[2]);
...
return (doRound ? (I$[3]||$incl$(3)).new3$F$F$F(Math.round(h * 10) / 10.0, Math.round(s * 1000) / 10.0, Math.round(p * 500) / 10.0) : (I$[3]||$incl$(3)).new3$F$F$F(h, s * 100, p * 50));
}, 1);
...
return (I$[3]||$incl$(3)).new3$F$F$F(Math.round(r * 255), Math.round(g * 255), Math.round(b * 255));
}, 1);
