(function(){var P$=Clazz.newPackage("java.awt.image"),I$=[['sun.awt.image.ImagingLib','java.awt.image.BufferedImage',['java.awt.geom.Point2D','.Float']]],$incl$=function(i){return I$[i]=Clazz.load(I$[0][i-1])};
...
throw Clazz.new_(Clazz.load('java.lang.IllegalArgumentException').c$$S,["Number of arrays in the  lookup table (" + numComponents + " is not compatible with the " + " src image: " + src ]);
...
throw Clazz.new_(Clazz.load('java.lang.IllegalArgumentException').c$$S,["Number of arrays in the  lookup table (" + numComponents + " is not compatible with the " + " src Raster: " + src ]);
}if ((I$[1]||$incl$(1)).filter$java_awt_image_RasterOp$java_awt_image_Raster$java_awt_image_WritableRaster(this, src, dst) != null ) {
return dst;
}if (Clazz.instanceOf(this.ltable, "java.awt.image.ByteLookupTable")) {
p$.byteFilter$java_awt_image_ByteLookupTable$java_awt_image_Raster$java_awt_image_WritableRaster$I$I$I.apply(this, [this.ltable, src, dst, width, height, numBands]);
} else {
