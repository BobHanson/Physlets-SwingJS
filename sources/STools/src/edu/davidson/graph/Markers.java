 ...
 */
      ...
      protected MarkerStyle[] styles;  // added by wc
 ...
 */
      public Markers() {
          last = 0;
          vert = new Vector[max];
          styles = new MarkerStyle[max];   // added by wc
      }
...
public void AddMarker( int m,int size, int type){
    AddMarker( m, 1, new boolean[] {false}, new int[]{0}, new int[]{0}) ;
    styles[m-1].type=type;
    styles[m-1].size=size;
}

public int getMarkerSize(int m){return styles[m-1].size; }
 ...
 */
     public void DeleteMarker( int n ) {
        ...
        if(n<1 || n>max) return;
        vert[n-1] = null;
        styles[n-1]=null; //added by W. Christian
   ...
   */
     public void ClearMarkers() {
        ...
        for(i=0; i<max; i++) {
           vert[i] = null;
           styles[i]=null;    // added by W. Christian
        }
...
*/
      ...
      public void drawCircle(Graphics g,double scale, int x, int y){
          int s=(int)Math.round(4*scale);
          g.fillOval(x-s,y-s,2*s+1,2*s+1);
      }

      public void drawSquare(Graphics g,double scale, int x, int y){
          int s=(int)Math.round(4*scale);
          g.fillRect(x-s,y-s,2*s+1,2*s+1);
      }
   ...
   */
      ...
      public void draw(Graphics g, int m, double scale, int x, int y) {
          ...
          // added by W. Christian
          if (styles[m-1].type==TYPE_CIRCLE){
              drawCircle(g,scale,x,y);
              return;
          } else if (styles[m-1].type==TYPE_SQUARE){
              drawSquare(g,scale,x,y);
              return;
          }
...
final class MarkerVertex extends Object {
      boolean draw;
      int     x;
      int     y;
}

final class MarkerStyle extends Object {
      int     size=4;
