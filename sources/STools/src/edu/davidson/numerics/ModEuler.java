...
public class ModEuler extends SODE {
    ...
    public int step(double dx, double x[]) {
        ...
        for(int i=0; i<numSteps ; i++) modEulerStep(h, x);
        if(dx-numSteps*h>tol){
            modEulerStep(dx-numSteps*h,  x);  // step by the remaining
            numSteps++;
        }
        return numSteps;
    ...
    private void modEulerStep(double stepSize, double x[]) {
            dydx=equations.rate(x);
            for(int i=0;i<numEqu; i++){
                x[i]=x[i]+stepSize*dydx[i];
                dydx=equations.rate(x);  // use each new value as it is obtained to get the rates
            }
    }
