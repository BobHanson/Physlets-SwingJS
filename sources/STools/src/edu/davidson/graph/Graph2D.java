...
// added by wc
    boolean showAxis=true;  // draw the axis on the plot.
...
// accessor methods added by wc
    ...
    public final boolean isShowAxis(){return showAxis;}
    public final void setShowAxis(boolean sa){showAxis=sa; return;}
                     ...
                     switch (c) {
                         ...
                         case ' ': case '\t':
                                if( nbytes > 0 ) {
                                   ...
                                   String s = new String(b,0,nbytes);  // changed by wc
...
*/
     public double getXmax() {
         ...
         if(dataset == null || dataset.isEmpty() ) return 1.0; // Changed from max;  by wc
 ...
 */
    ...
    //added by WC as accessor method
    public Color getDataBackground() {
        if(DataBackground == null)return DataBackground=this.getBackground();
        else return DataBackground;
    }

/**
 *  Added by wc.
 *  This method can be used to make last minute scale adjustments.  For example,
 *  it is uses to rescale the graph if analytic functions are present in SGraph.
*/
...
//        System.out.println("Graph2D paint method called!");
        ...
        paintFunctions(lg,r);  // added by W. Christian
...
*/
            if(dr.width > dr.height) {
                    x += (int)((dr.width-dr.height)/2.0);
                    width -= dr.width-dr.height;
            } else {
                    y += (int)((dr.height-dr.width)/2.0);
 ...
 */
        protected Rectangle drawAxis(Graphics g, Rectangle r) {
          try{   // try added by W. Christian to catch null pointer exception.
            Axis a;
            int waxis;
            Rectangle dr;
            int x;
            ...
            if(!showAxis) dr=r; else dr = getDataRectangle(g,r);  // added by wc
...
// Draw a frame around the data area (If requested)
            if(frame || !showAxis) drawFrame(g,x,y,width,height);
...
// data area.
            for (int i=0; i<axis.size(); i++) {
               ...
               switch (a.getAxisPos()) {
               case Axis.LEFT:
                          if(showAxis) r.x += a.width;       // all showAxis conditions added by wc
                          if(showAxis) r.width -= a.width;
               ...
               case Axis.RIGHT:
                          ...
                          a.drawzero  = false;
                           ...
                           break;
               case Axis.TOP:
                          if(showAxis) r.y += a.width;
                          if(showAxis) r.height -= a.width;
 ...
 */
      protected void drawFrame(Graphics g, int x, int y, int width, int height) {
        //Color c = g.getColor();    // changed to black by W. Christian
        Color c = Color.black;
        if( framecolor != null ) g.setColor(framecolor);
        if(showAxis)g.drawRect(x,y,width,height);
        else g.drawRect(x,y,width-1,height-1); // changed from drawRect(x,y,width,height) to get a nice frame by W. Christian
        g.setColor(c);
     }
 ...
 */
        public void run() {
           ...
           while(true) {
                ...
                draw = !draw;
                if(lg != null) lg.dispose(); // added by wc
              }
