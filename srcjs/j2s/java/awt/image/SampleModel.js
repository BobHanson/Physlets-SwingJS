...
var size = w * h;
if (w <= 0 || h <= 0 ) {
throw Clazz.new_(Clazz.load('java.lang.IllegalArgumentException').c$$S,["Width (" + w + ") and height (" + h + ") must be > 0" ]);
}if (size >= 2147483647 ) {
throw Clazz.new_(Clazz.load('java.lang.IllegalArgumentException').c$$S,["Dimensions (width=" + w + " height=" + h + ") are too large" ]);
}if (dataType < 0 || (dataType > 3 && dataType != 32 ) ) {
throw Clazz.new_(Clazz.load('java.lang.IllegalArgumentException').c$$S,["Unsupported dataType: " + dataType]);
}if (numBands <= 0) {
throw Clazz.new_(Clazz.load('java.lang.IllegalArgumentException').c$$S,["Number of bands must be > 0"]);
