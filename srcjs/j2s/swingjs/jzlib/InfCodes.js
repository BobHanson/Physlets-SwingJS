...
b = b>>>((this.tree[tindex + 1]));
k = k-((this.tree[tindex + 1]));
e = this.tree[tindex];
if (e == 0) {
...
this.len = this.len+((b & C$.inflate_mask[j]));
b = b>>(j);
k = k-(j);
...
this.dist = this.dist+((b & C$.inflate_mask[j]));
b = b>>(j);
k = k-(j);
this.mode = 5;
case 5:
