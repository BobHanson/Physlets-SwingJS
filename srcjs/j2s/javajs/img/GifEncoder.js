(function(){var P$=Clazz.newPackage("javajs.img"),I$=[['javajs.util.P3','javajs.util.M3','java.lang.Boolean','javajs.util.Lst','java.util.Hashtable',['javajs.img.GifEncoder','.ColorItem'],'javajs.util.CU',['javajs.img.GifEncoder','.ColorCell']]],$incl$=function(i){return I$[i]=Clazz.load(I$[0][i-1])};
...
{
C$.rgb2xyz = (I$[2]||$incl$(2)).newA9$FA(Clazz.array(Float.TYPE, -1, [0.4124, 0.3576, 0.1805, 0.2126, 0.7152, 0.0722, 0.0193, 0.1192, 0.9505]));
C$.xyz2rgb = (I$[2]||$incl$(2)).newA9$FA(Clazz.array(Float.TYPE, -1, [3.2406, -1.5372, -0.4986, -0.9689, 1.8758, 0.0415, 0.0557, -0.204, 1.057]));
}
;
...
}this.interlaced = ((I$[3]||$incl$(3)).TRUE === params.get$O("interlaced") );
...
switch ("maec".indexOf((params.get$O("captureMode")).substring(0, 1))) {
case 0:
...
this.looping = ((I$[3]||$incl$(3)).FALSE !== params.get$O("captureLooping") );
break;
case 2:
this.addHeader = false;
this.addImage = false;
...
if (this.addHeader) p$.writeHeader.apply(this, []);
this.addHeader = false;
if (this.addImage) {
p$.createPalette.apply(this, []);
p$.writeGraphicControlExtension.apply(this, []);
if (this.delayTime100ths >= 0 && this.looping ) p$.writeNetscapeLoopExtension.apply(this, []);
p$.writeImage.apply(this, []);
}});
...
this.doClose = false;
}if (this.capturing) this.params.put$TK$TV("captureByteCount", Integer.$valueOf(this.byteCount));
});

Clazz.newMeth(C$, 'createPalette', function () {
var tempColors = Clazz.new_((I$[4]||$incl$(4)));
var ciHash = Clazz.new_((I$[5]||$incl$(5)));
for (var i = 0, n = this.pixels.length; i < n; i++) {
var rgb = this.pixels[i];
var key = Integer.$valueOf(rgb);
var item = ciHash.get$O(key);
if (item == null ) {
item = Clazz.new_((I$[6]||$incl$(6)).c$$I$Z, [this, null, rgb, rgb == this.backgroundColor]);
...
ciHash = null;
var cells = p$.quantizeColors$javajs_util_Lst.apply(this, [tempColors]);
nColors = cells.size();
System.out.println$S("GIF final color count: " + nColors);
var colorMap = Clazz.new_((I$[5]||$incl$(5)));
this.bitsPerPixel = (nColors <= 2 ? 1 : nColors <= 4 ? 2 : nColors <= 16 ? 4 : 8);
this.palette = Clazz.array((I$[1]||$incl$(1)), [1 << this.bitsPerPixel]);
for (var i = 0; i < nColors; i++) {
var c = cells.get$I(i);
colorMap.put$TK$TV(Integer.$valueOf((I$[7]||$incl$(7)).colorPtToFFRGB$javajs_util_T3(this.palette[i] = c.setColor())), c);
}
this.pixels = p$.indexPixels$javajs_util_Lst$java_util_Map.apply(this, [cells, colorMap]);
});

Clazz.newMeth(C$, 'quantizeColors$javajs_util_Lst', function (tempColors) {
var n = tempColors.size();
var cells = Clazz.new_((I$[4]||$incl$(4)));
var cc = Clazz.new_((I$[8]||$incl$(8)).c$$I, [this, null, 0]);
cc.addLast$TV(Clazz.new_((I$[6]||$incl$(6)).c$$I$Z, [this, null, this.backgroundColor, true]));
cells.addLast$TV(cc);
cc = Clazz.new_((I$[8]||$incl$(8)).c$$I, [this, null, 1]);
...
if (n <= 256) {
cells.addLast$TV(cc);
cc = Clazz.new_((I$[8]||$incl$(8)).c$$I, [this, null, cells.size()]);
}}
tempColors.clear();
...
var w2 = this.width + 2;
var errors = Clazz.array((I$[1]||$incl$(1)), [w2]);
var newPixels = Clazz.array(Integer.TYPE, [this.pixels.length]);
var err = Clazz.new_((I$[1]||$incl$(1)));
var lab;
var rgb;
var nearestCell = Clazz.new_((I$[5]||$incl$(5)));
...
err = pe;
err.x = p$.clamp$F$F$F.apply(this, [err.x, -75, 75]);
err.y = p$.clamp$F$F$F.apply(this, [err.y, -75, 75]);
err.z = p$.clamp$F$F$F.apply(this, [err.z, -75, 75]);
lab.add$javajs_util_T3(err);
rgb = (I$[7]||$incl$(7)).colorPtToFFRGB$javajs_util_T3(this.toRGB$javajs_util_P3(lab));
}var key = Integer.$valueOf(rgb);
...
if (notLastCol) p$.addError$javajs_util_P3$I$javajs_util_P3A$I$I.apply(this, [err, 7, errors, p + 1, w2]);
if (notLastRow) {
if (j > 0) p$.addError$javajs_util_P3$I$javajs_util_P3A$I$I.apply(this, [err, 3, errors, p + this.width - 1, w2]);
p$.addError$javajs_util_P3$I$javajs_util_P3A$I$I.apply(this, [err, 5, errors, p + this.width, w2]);
if (notLastCol) p$.addError$javajs_util_P3$I$javajs_util_P3A$I$I.apply(this, [err, 1, errors, p + this.width + 1 , w2]);
}}err.x = 3.4028235E38;
...
p = p%(w2);
var errp = errors[p];
if (errp == null ) errp = errors[p] = Clazz.new_((I$[1]||$incl$(1)));
...
var lab = (I$[7]||$incl$(7)).colorPtFromInt$I$javajs_util_P3(rgb, null);
...
var xyz = (I$[1]||$incl$(1)).newP$javajs_util_T3(lab);
...
if (xyz == null ) xyz = Clazz.new_((I$[1]||$incl$(1)));
xyz.x = p$.sxyz$F.apply(this, [rgb.x]);
xyz.y = p$.sxyz$F.apply(this, [rgb.y]);
xyz.z = p$.sxyz$F.apply(this, [rgb.z]);
...
if (rgb == null ) rgb = Clazz.new_((I$[1]||$incl$(1)));
rgb.setT$javajs_util_T3(xyz);
rgb.scale$F(0.01);
C$.xyz2rgb.rotate$javajs_util_T3(rgb);
rgb.x = p$.clamp$F$F$F.apply(this, [p$.srgb$F.apply(this, [rgb.x]), 0, 255]);
rgb.y = p$.clamp$F$F$F.apply(this, [p$.srgb$F.apply(this, [rgb.y]), 0, 255]);
rgb.z = p$.clamp$F$F$F.apply(this, [p$.srgb$F.apply(this, [rgb.z]), 0, 255]);
return rgb;
});
...
if (lab == null ) lab = Clazz.new_((I$[1]||$incl$(1)));
var x = p$.flab$F.apply(this, [xyz.x / 95.0429]);
var y = p$.flab$F.apply(this, [xyz.y / 100]);
var z = p$.flab$F.apply(this, [xyz.z / 108.89]);
lab.x = (116 * y) - 16;
lab.y = 500 * (x - y);
lab.z = 200 * (y - z);
return lab;
...
var x = xyz.y / 500 + y;
var z = y - xyz.z / 200;
xyz.x = p$.fxyz$F.apply(this, [x]) * 95.0429;
xyz.y = p$.fxyz$F.apply(this, [y]) * 100;
xyz.z = p$.fxyz$F.apply(this, [z]) * 108.89;
return xyz;
});
...
p$.putWord$I.apply(this, [this.width]);
p$.putWord$I.apply(this, [this.height]);
this.putByte$I(0);
this.putByte$I(0);
this.putByte$I(0);
});
...
p$.putWord$I.apply(this, [this.delayTime100ths > 0 ? this.delayTime100ths : 0]);
this.putByte$I(0);
this.putByte$I(0);
}});
...
this.curx = 0;
if (this.interlaced) p$.updateY$I$I.apply(this, [C$.INTERLACE_PARAMS[this.pass], C$.INTERLACE_PARAMS[this.pass + 4]]);
 else ++this.cury;
...
this.bufPt = 0;
var ent = p$.nextPixel.apply(this, []);
var hshift = 0;
var fcode;
for (fcode = this.hsize; fcode < 65536; fcode = fcode*(2)) ++hshift;

hshift = 8 - hshift;
var hsizeReg = this.hsize;
p$.clearHash$I.apply(this, [hsizeReg]);
p$.output$I.apply(this, [this.clearCode]);
var c;
outer_loop : while ((c = p$.nextPixel.apply(this, [])) != -1){
...
if (i == 0) disp = 1;
do {
if ((i = i-(disp)) < 0) i = i+(hsizeReg);
if (this.htab[i] == fcode) {
ent = this.codetab[i];
continue outer_loop;
}} while (this.htab[i] >= 0);
}p$.output$I.apply(this, [ent]);
ent = c;
if (this.freeEnt < this.maxmaxcode) {
this.codetab[i] = this.freeEnt++;
this.htab[i] = fcode;
} else {
p$.clearBlock.apply(this, []);
}}
p$.output$I.apply(this, [ent]);
p$.output$I.apply(this, [this.EOFCode]);
});

Clazz.newMeth(C$, 'output$I', function (code) {
this.curAccum = this.curAccum&(this.masks[this.curBits]);
if (this.curBits > 0) this.curAccum = this.curAccum|((code << this.curBits));
 else this.curAccum = code;
this.curBits = this.curBits+(this.nBits);
while (this.curBits >= 8){
p$.byteOut$B.apply(this, [($b$[0] = ((this.curAccum & 255)|0), $b$[0])]);
this.curAccum = this.curAccum>>(8);
this.curBits = this.curBits-(8);
}
...
p$.clearHash$I.apply(this, [this.hsize]);
this.freeEnt = this.clearCode + 2;
this.clearFlag = true;
p$.output$I.apply(this, [this.clearCode]);
});
...
this.byteCount = this.byteCount+(this.bufPt);
this.bufPt = 0;
}});
...
this.setT$javajs_util_T3(this.this$0.toLABnorm$I(rgb));
}, 1);
