...
public class DataGenerator implements Runnable {
	...
	void setDataArray(double[][] dataArray) {
		if (dataArray == null && externalData == null)
			return;
		newData = true;
		newDataArray = dataArray;
		if(surfaceCanvas.isJS)externalData = newDataArray;
		if (dataArray != null)
			createData();
	}
	...
	void doCalc() {
		...
		if (f1 && externalData == null) {
			...
			if (parser1.getErrorCode() != Parser.NO_ERROR) {
				surfaceCanvas.setMessage("Parse error: " + parser1.getErrorString() + " at function 1, position "
						+ parser1.getErrorPosition());
		...
		if (f2) {
			...
			if (parser2.getErrorCode() != Parser.NO_ERROR) {
				surfaceCanvas.setMessage("Parse error: " + parser2.getErrorString() + " at function 2, position "
						+ parser2.getErrorPosition());
		...
		if (externalData != null
				&& ((calc_divisions + 1 != externalData.length) || (calc_divisions + 1 != externalData[0].length))) {
			// System.out.println("Wrong array length: exiting doCalc in DataGenerator.");
			return;
		}

		// the following is not needed since we have to vertex matrices and one of them
		// should always be valid.
		// surfaceCanvas.setDataAvailability(false);

		// func1calc = f1; func2calc = f2;
		// if (func1calc)func1name = new
		// String(function_panel.getFunction1Definition());
		// if (func2calc) func2name = new
		// String(function_panel.getFunction2Definition());
		...
		// if(true || newMemory!=totalMemory || vertex==null){ // Changed by W.
		// Christian
		if (true) {
		...
		while (i <= calc_divisions) {
			...
			while (j <= calc_divisions) {
				...
				if (showf1) {
					if (externalData != null)
						v = (float) externalData[i][j]; // are things flipped?
					else if (f1)
					...
					vertex[0][k] = new SurfaceVertex((x - xi) * xfactor - 10, (y - yi) * yfactor - 10, v,
							surfaceCanvas);
				}
				if (f2) {
					...
					vertex[1][k] = new SurfaceVertex((x - xi) * xfactor - 10, (y - yi) * yfactor - 10, v,
							surfaceCanvas);
				}
				j++;
				y += stepy;
	 ...
	 *
	 * Call when the appelt is unloaded from memeory and the object is no longer
	 * needed.
	 *
	 */
